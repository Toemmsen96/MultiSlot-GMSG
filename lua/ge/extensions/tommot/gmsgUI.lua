...
local logLevelSelected = imgui.IntPtr(2) -- Default to Debug level
    ...
    if settings ~= nil then
        ...
        for key, value in pairs(settingsMap) do
            if settings[key] ~= nil then
                ...
                gmsg.logToConsole('I', "loadSettings", "Loaded setting: " .. key .. " = " .. tostring(settings[key]))
            end
        end
    end 
    if settings == nil then
...
local function render()
    ...
    if imgui.BeginTabBar("Tabs") then
        if imgui.BeginTabItem("Generate Standalone") then
            ...
            outputPath = ffi.string(outputPathBuffer) -- Update outputPath with the modified value
        ...
        if imgui.BeginTabItem("Generate Manually") then
            ...
            if imgui.Button("Generate MultiSlot-Mods from Additional Mods concurrently") then
                extensions.load("tommot_additionalToMultiSlot")
                core_jobsystem.create(tommot_additionalToMultiSlot.additionalToMultiSlotJob, tonumber(concurrencyDelay[0]))
            end
            if imgui.IsItemHovered() then
